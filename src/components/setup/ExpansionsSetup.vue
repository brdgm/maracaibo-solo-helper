...
export default defineComponent({
  ...
  methods: {
    ...
    toggleExpansion(expansion : Expansion) : void {
      ...
      if (expansion == Expansion.FIRELAND) {
        this.state.setup.expansions = this.state.setup.expansions.filter(item => item != Expansion.OCEANIA)
      }
      if (expansion == Expansion.OCEANIA) {
        this.state.setup.expansions = this.state.setup.expansions.filter(item => item != Expansion.FIRELAND)
      }
    }
  }
})
